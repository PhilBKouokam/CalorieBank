...
vi.mock('../../mobile/lib/native-health/index.android', () => ({ nativeHealthQualification: { access: h.access, inspect: h.inspect, openSettings: h.settings, cancel: h.cancel } }));
...
describe('Android developer qualification UI', () => {
  it('renders safe counts, exact origins and withheld burn, with accessible controls', async () => {
    ...
    for (const button of screen!.root.findAll((n) => String(n.type) === 'Pressable')) expect(button.props.accessibilityRole).toBe('button');

...
import { nativeHealthQualification } from '../lib/native-health/index.android';
...
function QualificationDiagnostics() {
  ...
  useFocusEffect(useCallback(() => {
    let active = true;
    void nativeHealthQualification.access().then((value) => { if (active) setAccess(value); });
    const subscription = AppState.addEventListener('change', (state) => {
      nativeHealthQualification.cancel(); setReport(null);
      if (state === 'active') void nativeHealthQualification.access().then((value) => { if (active) setAccess(value); });
    });
    return () => { active = false; subscription.remove(); nativeHealthQualification.cancel(); };
  }, []));
  async function run(action: 'permissions' | 'read' | 'settings') {
    if (busy) return;
    setBusy(true); setReport(null); setMessage('');
    try {
      if (action === 'permissions') setAccess(await nativeHealthQualification.access(true));
      else if (action === 'settings') { if (!await nativeHealthQualification.openSettings()) setMessage('Health Connect settings could not open.'); }
      else { const result = await nativeHealthQualification.inspect(origin); setAccess(result.access); setReport(result); }
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content}>
    ...
    <Text>Read-only test tool. Health Connect does not update your bank in this build.</Text>
    ...
    {report?.origins.map((source) => <Pressable key={source.id} disabled={busy} accessibilityRole="button" style={styles.button} onPress={() => { setOrigin(source.id); setReport(null); }}><Text>{source.id}</Text></Pressable>)}

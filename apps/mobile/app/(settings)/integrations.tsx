...
function ServiceSheet({ appleBurnState, busy, connections, message, messageTone, name, onChange, onClose, onDisconnect, onReconnect, onRefresh }: { appleBurnState: AppleHealthBurnState; busy: string | null; connections: HealthConnectionsResponse | null; message: string | null; messageTone: 'attention' | 'error' | 'success'; name: ServiceName | null; onChange: (role: Role) => void; onClose: () => void; onDisconnect: (name: 'Fitbit' | 'FatSecret') => void; onReconnect: (name: 'Fitbit' | 'FatSecret') => void; onRefresh: (name: ServiceName) => void }) {
  ...
  return <Sheet onClose={onClose} visible={name !== null}><SheetHeader onClose={onClose} title={name ?? ''} /><Text style={[styles.serviceStatus, needsAttention && styles.attention]}>{name === 'Apple Health' ? 'Managed by iOS' : needsAttention ? 'Needs attention' : 'Connected'}</Text><View style={styles.detailBlock}><Text style={styles.detailLabel}>Used for</Text><Text style={styles.detailValue}>{usage || 'Not currently used'}</Text></View>
    ...
    {__DEV__ && name === 'Apple Health' ? <Link href="/health-diagnostics" asChild><Pressable accessibilityRole="button" style={styles.devButton}><Text style={styles.devText}>Diagnostics</Text></Pressable></Link> : null}

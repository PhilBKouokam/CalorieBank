...
import { nativeIntake, nativeHealthCapability, connectNativeHealth, getNativeHealthDiagnostics, discoverNativeIntakeWriters, type NativeIntakeWriter as AppleHealthIntakeWriter, getNativeHealthConnectionStatus, refreshNativeHealthForCurrentAccount, syncNativeHealthToday } from '@/lib/native-health';
...
export default function IntegrationsScreen() {
  ...
  const [nativeFoodAccess, setNativeFoodAccess] = useState(false);
  ...
  const load = useCallback(async (showLoading = false) => {
    ...
    if (generation !== loadGeneration.current) return;
    setNativeFoodAccess(nativeAccess?.granted.includes('nutrition') ?? false);
  ...
  useEffect(() => {
    if (!nativeIntake.supported) return;
  ...
  const displayConnections = useMemo(() => {
    return connections ? connectionsForNativeCapability(nativeHealthCapability.supported ? composeAppleHealthConnections(connections, appleState, appleBurnState) : connections, nativeHealthCapability.supported, nativeIntake.supported, nativeFoodAccess) : null;
  }, [appleBurnState, appleState, connections, nativeFoodAccess]);
  ...
  async function connectFatSecretForEaten() {
    ...
    try {
      ...
      const feedback = await refreshFatSecretWithFeedback(timezone());
      closeSheets();
      setMessageTone(feedback.state === 'ready' ? 'success' : 'attention');
  ...
  async function refreshService(name: ServiceName, role: Role = serviceRole) {
    ...
    try {
      ...
      if (name === 'FatSecret' && connections?.eaten.selected?.label === 'FatSecret') {
        const feedback = await refreshFatSecretWithFeedback(timezone());
        setMessageTone(feedback.state === 'ready' ? 'success' : 'attention');
        setMessage(feedback.message);
        await load();
    ...
    <SafeAreaView edges={['bottom']} style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <InventorySection
          ...
          onManage={(item) => { if (item.transportLabel === 'Health Connect') { openNativeFood(); return; } operations.current.invalidate(); setMessage(null); setServiceRole('eaten'); setService(item.serviceName); }}
          onRemove={removeFromInventory}
          role="eaten"
...
function buildInventory(role: Role, connections: HealthConnectionsResponse | null): InventoryItem[] {
  ...
  const roleOptions = [roleState.selected, ...roleState.alternatives]
    .filter((option): option is HealthConnectionOption => option !== null && (nativeHealthCapability.supported || !option.deviceManaged || (role === 'eaten' && nativeIntake.supported && option.transportLabel === 'Health Connect')));
...
function RoleSelector({ onNativeDetails, onReconnect, appleBurnState, busy, data, message, messageTone, onAdd, onAppleDetails, onClose, onRefreshApple, onSelect, role }: { onNativeDetails: () => void; onReconnect: (name: 'Fitbit' | 'FatSecret') => void; appleBurnState: AppleHealthBurnState; busy: boolean; data: HealthConnectionsResponse[Role] | null; message: string | null; messageTone: 'attention' | 'error' | 'success'; onAdd: () => void; onAppleDetails: () => void; onClose: () => void; onRefreshApple: () => void; onSelect: (option: HealthConnectionOption) => void; role: Role | null }) {
  const options = data ? trackerChoices(data.alternatives, data.selected) : [];
  return <Sheet onClose={onClose} visible={role !== null}>
    ...
    {options.map((option) => {
      ...
      const localEquivalent = remoteHint ? data?.alternatives.find((candidate) => candidate.label === option.label
        && candidate.deviceManaged && !crossDeviceSourceHint(candidate, nativeHealthCapability.supported)) : undefined;
      const manageOption = localEquivalent ?? option;
      ...
      const unavailableCopy = option.deviceManaged && !nativeHealthCapability.supported && option.transportLabel !== 'Health Connect'
        ...
        : appleBurnState === 'refresh_failed' && role === 'burned' && option.deviceManaged
          ? 'Refresh failed'
          : 'Needs refresh';
      return <View key={option.optionId} style={styles.optionRow}>
        {selected ? <Ionicons color={colors.primary} name="checkmark" size={22} /> : <View style={{ width: 22 }} />}
        ...
        {nativeIntake.supported && manageOption.transportLabel === 'Health Connect' ? <Pressable accessibilityRole="button" disabled={busy} onPress={onNativeDetails} style={styles.inlineAction}><Text style={styles.actionText}>Manage</Text></Pressable> : option.primaryAction === 'reconnect' && (option.label === 'Fitbit' || option.label === 'FatSecret') ?
          ...
          !crossDeviceSourceHint(manageOption, nativeHealthCapability.supported) && nativeHealthCapability.supported && (noBurnData || (role === 'eaten' && manageOption.deviceManaged)) ?
    ...
    {role === 'eaten' || data?.canAddSource || options.length === 0 ? <Pressable accessibilityRole="button" disabled={busy} onPress={onAdd} style={styles.addRow}><Ionicons color={colors.primary} name="add" size={22} /><Text style={styles.addText}>{options.length === 0 ? 'Choose a source' : 'Add another source'}</Text></Pressable> : null}
...
function ServiceSheet({ role, onChooseTracker, appleBurnState, busy, connections, message, messageTone, name, onChange, onClose, onDiagnostics, onDisconnect, onReconnect, onRefresh }: { role: Role; onChooseTracker: () => void; appleBurnState: AppleHealthBurnState; busy: string | null; connections: HealthConnectionsResponse | null; message: string | null; messageTone: 'attention' | 'error' | 'success'; name: ServiceName | null; onChange: (role: Role) => void; onClose: () => void; onDiagnostics: () => void; onDisconnect: (name: 'Fitbit' | 'FatSecret') => void; onReconnect: (name: 'Fitbit' | 'FatSecret') => void; onRefresh: (name: ServiceName) => void }) {
  ...
  return <Sheet onClose={onClose} visible={name !== null}><SheetHeader onClose={onClose} title={name ?? ''} /><Text style={[styles.serviceStatus, needsAttention && styles.attention]}>{name === 'Apple Health' ? 'Managed by iOS' : needsAttention ? 'Needs attention' : 'Connected'}</Text><View style={styles.detailBlock}><Text style={styles.detailLabel}>Used for</Text><Text style={styles.detailValue}>{usage || 'Not currently used'}</Text></View>
    {name === 'Apple Health' ? <>
      {role === 'burned' ? <><View style={styles.detailBlock}><Text style={styles.detailLabel}>Burn data</Text><Text style={styles.detailValue}>{appleBurnState === 'ready' ? 'Available' : appleBurnState === 'refreshing' ? 'Refreshing…' : appleBurnState === 'refresh_failed' ? 'Refresh failed' : appleBurnState === 'no_burn_data' ? 'Not available yet' : 'Needs refresh'}</Text></View>
      {appleBurnState === 'no_burn_data' ? <Text style={styles.body}>CalorieBank can access Apple Health, but no complete calorie-burn data was found.</Text> : null}</> : <><View style={styles.detailBlock}><Text style={styles.detailLabel}>Food tracker</Text><Text style={styles.detailValue}>{[connections?.eaten.selected, ...(connections?.eaten.alternatives ?? [])].find((option) => option?.deviceManaged)?.label ?? 'Choose a food tracker'}</Text></View><Pressable accessibilityRole="button" disabled={busy !== null} onPress={onChooseTracker} style={styles.secondaryButton}><Text style={styles.secondaryButtonText}>Choose food tracker</Text></Pressable></>}
...
const styles = StyleSheet.create({
  ...
  backdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.34)', justifyContent: 'flex-end', padding: spacing.md }, sheet: { backgroundColor: colors.surface, borderRadius: radii.lg, gap: spacing.sm, maxHeight: '82%', padding: spacing.lg }, sheetHeader: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginBottom: spacing.sm }, sheetTitle: { color: colors.text, flex: 1, fontSize: typography.heading, fontWeight: '800' }, optionRow: { alignItems: 'center', flexDirection: 'row', flexWrap: 'wrap', gap: spacing.md, minHeight: 56, paddingVertical: spacing.sm }, optionText: { flexGrow: 1, flexBasis: 160, minWidth: 0 }, optionLabel: { color: colors.text, fontSize: typography.body, fontWeight: '700' }, optionDetail: { color: colors.textMuted, fontSize: typography.caption, marginTop: spacing.xs }, inlineAction: { alignItems: 'center', justifyContent: 'center', minHeight: 44, paddingHorizontal: spacing.sm }, addRow: { alignItems: 'center', borderTopColor: colors.border, borderTopWidth: StyleSheet.hairlineWidth, flexDirection: 'row', gap: spacing.md, minHeight: 56, marginTop: spacing.xs }, addText: { flex: 1, color: colors.primary, fontSize: typography.body, fontWeight: '800' }, cancelButton: { alignItems: 'center', justifyContent: 'center', minHeight: 44 }, cancelText: { color: colors.textMuted, fontSize: typography.body, fontWeight: '700' }, attentionText: { color: colors.accent, fontSize: typography.body, lineHeight: 22 }, errorText: { color: colors.danger, fontSize: typography.body, lineHeight: 22 }, successText: { color: colors.primaryDark, fontSize: typography.body, lineHeight: 22 }, sheetSpinner: { marginVertical: spacing.sm },

...
/** Setup chooses an exact observed writer; settings keeps its advanced controls. */
export function AndroidFoodChoices({ onSelected, onBusyChange }: { onSelected: () => Promise<void>; onBusyChange: (busy: boolean) => void }) {
  ...
  async function choose(id: string | null) {
    ...
    changeBusy(true); setRequested(id); setMessage(''); setSources([]);
    try {
  ...
  return <View style={styles.list}>
    {trackers.map((tracker) => <Pressable key={tracker.id} accessibilityLabel={`Choose ${tracker.name}`} accessibilityRole="button" disabled={busy} onPress={() => void choose(tracker.id)} style={styles.option}>
      <Text style={styles.name}>{tracker.name}</Text><Text style={styles.detail}>via Health Connect</Text>
    </Pressable>)}
    <Pressable accessibilityRole="button" disabled={busy} onPress={() => void choose(null)} style={styles.option}>
    ...
    {sources.map((source, index) => <Pressable key={source.source.id} accessibilityRole="button" disabled={busy} style={styles.option} onPress={() => {
      if (locked.current) return;
      changeBusy(true);
    ...
    {message ? <View style={styles.list}><Text accessibilityLiveRegion="polite" style={styles.detail}>{message}</Text>
      <Pressable accessibilityRole="button" disabled={busy} onPress={() => void choose(requested)} style={styles.option}><Text style={styles.name}>Check again</Text></Pressable>
